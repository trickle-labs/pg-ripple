...
| v0.103.0 | **Domain rule library infrastructure** — rule library format specification: a Turtle file containing Datalog rules, SHACL shapes, and required metadata triples (`dcterms:title`, `dcterms:license`, `dcterms:description`, `owl:versionInfo`); `_pg_ripple.rule_libraries` catalog table `(name TEXT, version TEXT, installed_at TIMESTAMPTZ, description TEXT, license_iri TEXT, dependencies TEXT[])`; `install_rule_library(source TEXT)` (URL or local path), `upgrade_rule_library(name TEXT)`, `uninstall_rule_library(name TEXT)` SQL functions with dependency resolution and conflict checking; `install_rule_library` surfaces the library's `dcterms:license` and `dcterms:description` to the operator and requires explicit confirmation before loading; implicit dependency activation; REST endpoint `GET /rule-libraries` listing installed libraries; no domain-specific libraries are bundled in-tree — instead a documentation chapter (`docs/src/cookbook/rule-libraries.md`) explains the format, how to author and publish a library, and what licence and disclaimer requirements operators should evaluate before installing third-party content (see [plans/expert-system.md §5](plans/expert-system.md) for the full rationale) | Planned | Medium | — |
| v0.104.0 | **Guided rule authoring & LLM rule extraction** — `draft_rule_from_nl(description TEXT)` SQL function translating a natural language rule description to Datalog via the LLM endpoint (`pg_ripple.llm_endpoint`), returning a draft rule string with a preview of derivations it would produce on sample data before committing; `validate_rule(rule TEXT)` checks syntax, detects unused variables, and identifies potential stratification issues; `suggest_rules(graph_iri TEXT, examples JSONB)` identifies statistical patterns in observed triples and proposes candidate Datalog rules for review; REST endpoint `POST /rules/draft` in `pg_ripple_http` exposing `draft_rule_from_nl` and `validate_rule` for integration into external tooling | Planned | Large | — |
