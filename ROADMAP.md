...
| v0.103.0 | **Domain rule libraries** — rule library format specification (Turtle file containing Datalog rules, SHACL shapes, and metadata triples); `_pg_ripple.rule_libraries` catalog table `(name TEXT, version TEXT, installed_at TIMESTAMPTZ, description TEXT, dependencies TEXT[], license_iri TEXT)`; `install_rule_library(name TEXT, source TEXT)`, `upgrade_rule_library(name TEXT)`, `uninstall_rule_library(name TEXT)` SQL functions with dependency resolution and conflict checking; `install_rule_library` surfaces the `dcterms:license` IRI to the operator before completing installation; implicit dependency activation (installing `'aml-eu'` auto-activates `'foaf'` and `'dcterms'`); first three reference libraries shipped in-tree: `'medical-decision-support-v1'` (clinical risk stratification rules and SHACL shapes), `'aml-eu-v1'` (EU AML Directive IV/V decision logic), `'gdpr-compliance-v1'` (data processing lawfulness and erasure rules) — all three carry a mandatory `dcterms:description` disclaimer stating the library is illustrative and does not constitute legal or medical advice; REST endpoint `GET /rule-libraries` listing available and installed libraries; documentation chapter including licensing and liability guidance (see [plans/expert-system.md §5](plans/expert-system.md) for the full licensing analysis) | Planned | Large | — |
