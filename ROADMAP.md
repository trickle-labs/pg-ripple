...
| v0.106.0 | **Temporal reasoning — Phase 1: VP schema & basic operators** — add `valid_from TIMESTAMPTZ` and `valid_to TIMESTAMPTZ` columns to VP tables and all VP creation/promotion helpers (HTAP delta/main split, rare-predicate path, `promote_predicate`); `_pg_ripple.temporal_facts` overflow table; migration script updating all existing VP tables; temporal operators in Datalog parser: `AFTER(timestamp)`, `BEFORE(timestamp)`, `DURING(from, to)` compiling to tsrange queries; `pg_ripple.enable_temporal_operators` GUC; `pg:temporal_window(?subject, ?predicate, ?start, ?end)` SPARQL function; `sh:validFor` SHACL constraint with XSD duration expression; pg_regress tests for operator correctness and migration round-trip | Planned | Large | — |
| v0.107.0 | **Temporal reasoning — Phase 2: sequential patterns & CDC integration** — temporal operators: `WITHIN(duration)`, `SEQUENCE(event1, event2, window)`, `CONSECUTIVE(n, predicate, window)` compiling to PostgreSQL window functions over temporally-ordered fact streams; integrate with CDC replication log to maintain `valid_from`/`valid_to` intervals automatically as facts are asserted and retracted; temporal pattern detection rules: "if condition holds for N consecutive readings within window W, then conclude C" | Planned | Very Large | — |
...
v0.106.0       ─── Expert system platform — phase 7a: temporal reasoning (basic):
               │   valid_from/valid_to VP schema + migration, AFTER/BEFORE/DURING
               │   operators, pg:temporal_window() SPARQL function, sh:validFor
       │
v0.107.0       ─── Expert system platform — phase 7b: temporal reasoning (advanced):
               ...
               │   CDC integration for automatic validity interval maintenance
       │
