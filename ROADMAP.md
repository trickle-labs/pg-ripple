...
| [v0.128.0](roadmap/v0.128.0.md) | **JSON mapping relational writeback — JSON-LD reverse mapping (RDF → Relational)** — completes the `register_json_mapping` round-trip by adding a relational write-back path so that RDF knowledge-graph changes can be automatically propagated back to the source relational table; **(schema)** add five new columns to `_pg_ripple.json_mappings`: `writeback_table TEXT`, `writeback_schema TEXT NOT NULL DEFAULT 'public'`, `writeback_key_columns TEXT[] NOT NULL DEFAULT '{}'`, `writeback_conflict_policy TEXT NOT NULL DEFAULT 'replace' CHECK (writeback_conflict_policy IN ('replace','skip','error'))`, `writeback_enabled BOOLEAN NOT NULL DEFAULT false`; add catalog table `_pg_ripple.json_writeback_queue (id BIGSERIAL PRIMARY KEY, mapping_name TEXT NOT NULL REFERENCES _pg_ripple.json_mappings(name) ON DELETE CASCADE, subject_id BIGINT NOT NULL, operation TEXT NOT NULL CHECK (operation IN ('upsert','delete')), queued_at TIMESTAMPTZ NOT NULL DEFAULT now(), processed_at TIMESTAMPTZ, error TEXT)`; **(SQL API)** `pg_ripple.register_json_mapping()` gains four optional new parameters `writeback_table`, `writeback_schema`, `writeback_key_columns`, `writeback_conflict_policy` (all backward-compatible defaults); `pg_ripple.writeback_json_row(mapping TEXT, subject_iri TEXT) → BIGINT` derives a JSON payload for the subject using the stored JSON-LD context, maps each JSON key to a relational column matching the `writeback_table` schema via `information_schema.columns`, and executes `INSERT INTO <writeback_schema>.<writeback_table> (…) VALUES (…) ON CONFLICT (<writeback_key_columns>) DO UPDATE SET …` (`'replace'`), `DO NOTHING` (`'skip'`), or raises `PT0551 ERROR: json mapping writeback conflict` (`'error'`); raises `PT0550 ERROR: json mapping writeback target not configured` when `writeback_table` is NULL; returns rows affected; `pg_ripple.writeback_json_row_delete(mapping TEXT, subject_iri TEXT) → BIGINT` decodes key-column values from VP tables and executes `DELETE FROM <target> WHERE <key_cols> = …`; `pg_ripple.enable_json_writeback(mapping TEXT) → VOID` validates `writeback_table` exists and `writeback_key_columns` is non-empty, sets `writeback_enabled = true`, and installs `AFTER INSERT OR DELETE FOR EACH ROW` trigger `_pg_ripple.jwb_<mapping>_<predicate_id>` on each `_pg_ripple.vp_*_delta` table corresponding to a predicate IRI in the mapping context — each trigger fires `_pg_ripple.json_writeback_enqueue_fn()` which inserts into `json_writeback_queue`; background merge-worker tick drains the queue in batches of `pg_ripple.json_writeback_batch_size` (GUC, default 100) by calling `writeback_json_row()` / `writeback_json_row_delete()`; `pg_ripple.disable_json_writeback(mapping TEXT) → VOID` sets `writeback_enabled = false` and drops all associated `_pg_ripple.jwb_*` triggers; `pg_ripple.json_writeback_status() → TABLE(mapping_name TEXT, pending BIGINT, errors BIGINT, last_error TEXT, last_processed_at TIMESTAMPTZ)` — operational view over the queue; **(HTTP)** `POST /json-mapping/{name}/writeback` with body `{"subject_iri": "…"}` — synchronous single-subject writeback, returns `{"rows_affected": N}`; `GET /json-mapping/{name}/writeback/status` returns queue depth and last error JSON; **(tests)** 20 pg_regress tests: full round-trip (ingest JSON → VP → `writeback_json_row()` → `SELECT` from `contacts_test`); upsert-on-conflict updates existing row; `skip` conflict policy leaves row unchanged; `error` conflict policy raises PT0551; `writeback_json_row_delete` removes row; `enable_json_writeback` installs VP triggers; queue drain by merge-worker tick; `json_writeback_status()` accuracy before and after drain; `disable_json_writeback` drops triggers; `PT0550` raised when `writeback_table` not configured; `PT0550` raised when `writeback_key_columns` empty; backward compatibility — existing `register_json_mapping()` calls without new params unchanged; multi-subject bulk writeback via `SELECT pg_ripple.writeback_json_row(mapping, s) FROM subjects`; HTTP `/writeback` endpoint returns correct `rows_affected`; schema mismatch (missing column) raises error with context; `writeback_schema` non-public target; `writeback_conflict_policy = 'error'` does not write partial rows; `json_writeback_batch_size` GUC respected; `feature_status()` includes `'json_mapping_writeback'` entry; **(docs)** blog post `blog/json-ld-reverse-mapping.md` covering the relational writeback pattern, trigger-based automation, conflict policies, and the end-to-end round-trip use case (REST API → RDF knowledge graph → relational write-back); update `docs/src/features/json-mapping.md` with writeback section and conflict-policy table; **(migration)** add migration script `sql/pg_ripple--0.127.0--0.128.0.sql` adding new `json_mappings` columns and `json_writeback_queue` table | Planned | Large | [Full details](roadmap/v0.128.0.md) |
