...
-- Uses only pre-existing IRIs so no new dictionary entries are created and the
-- VP sequence numbers seen by downstream tests (e.g. datalog_cwb_interaction)
-- remain stable.
...
-- Load source triple into the named graph that the rule watches.
-- Carol/Dave were previously inserted and deleted in CWB-02/CWB-03.
SELECT pg_ripple.load_ntriples_into_graph(
    '<https://cwb.test/Carol> <https://cwb.test/knows> <https://cwb.test/Dave> .',
    'https://cwb.test/source'
) = 1 AS ivm02_source_loaded;

SELECT pg_ripple.refresh_construct_rule('cwb_ivm02_conf') >= 1 AS ivm02_recompute_ok;
