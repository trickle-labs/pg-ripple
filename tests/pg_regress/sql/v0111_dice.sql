...
-- Rule head must be a 3-term RDF triple; body patterns are comma-separated.
-- Note: decimal literals (e.g. 0.85) cannot be used directly because the
-- Datalog tokenizer uses '.' as a rule terminator. Use integer thresholds
-- (e.g. > 0 for any overlap, = 1 for identical filters).

SELECT pg_ripple.load_rules($$
?x <http://example.org/candidate> ?y :- ?x <http://example.org/nameBloom> ?bx , ?y <http://example.org/nameBloom> ?by , pg:dice_similarity(?bx, ?by) > 0 .
