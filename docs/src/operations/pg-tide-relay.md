...
> IVM only. All relay functionality described here requires pg_tide.
...
Relay examples require pg_tide and pg_ripple in the same database. Install
pg_trickle as well only when you also use pg_ripple's IVM-backed live views.
If both companion extensions are installed, create pg_tide before pg_trickle so
`pgtrickle.attach_outbox()` can call `tide.outbox_create()`:
...
calling relay functions. Call `pg_ripple.pg_trickle_available()` before calling
IVM-backed view functions.
...
`tide.relay_set_inbox_v2()`. Here we subscribe to the `iot.sensors` Kafka
...
pg_ripple side — just register extra `relay_set_outbox_v2` pipelines:
...
The framed output is a clean, nested JSON-LD document ready to publish through
the pg_tide outbox:
...
pg_tide and pg_ripple run in the same PostgreSQL instance. The `pg-tide`
relay process is separate and stateless. A single relay instance handles **both
...
(`tide.relay_set_outbox_v2()` / `tide.relay_set_inbox_v2()`), and the relay reads it on
...
- Use **pg_tide's retention drain** (`tide.outbox_create(p_retention_hours => N)`) to cap
