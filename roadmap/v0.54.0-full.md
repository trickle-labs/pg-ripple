...
- [ ] **Publish to GHCR**: `ghcr.io/grove/pg-ripple:0.54.0` with all extensions pre-installed; tag at every release via GitHub Actions
...
- [ ] **Extension image**: publish `ghcr.io/grove/pg-ripple:<version>-cnpg` built from `docker/Dockerfile.cnpg` — a minimal image containing compiled `.so` and SQL files for pg_ripple, pg_trickle, PostGIS, and pgvector at `/var/lib/postgresql/extension-files/`; tagged at every release via GitHub Actions
